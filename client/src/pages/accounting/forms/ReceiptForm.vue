...
import ReceiptFormSchema from './schemas/ReceiptFormSchema';
import { getCustomers } from '@/services/customer.service';
import { createReceipt, updateReceipt } from '@/services/receipt.service';
    ...
    methods: {
        ...
        submitForm() {
            ...
            if(this.submitMode=="update") {
                receipt._id = this.loadedEntityId;
                updateReceipt(this.loadedEntityId, receipt).then(response => {
                    var saveActionStatus = response;
                    this.$emit('saved', saveActionStatus);
                });
            } else if (this.submitMode == "create") {
                createReceipt(receipt).then(response => {
