                ...
                <div class="card-body" v-else>
                    ...
                    <b-tabs class="tickets-tab-switch">
                        <b-tab title="Overview" active>
                            ...
                            <div class="mx-4">
                                <EmployeeTransactionForm v-if="editMode" :form-data="loadedEntity" :submit-mode="submitMode" 
                                    @saved="saved" @cancelForm="editCancelled"></EmployeeTransactionForm>
...
import { getEmployeeTransactionById } from "@/services/employeeTransaction.service";
import EmployeeTransactionForm from '@/pages/accounting/forms/EmployeeTransactionForm.vue';
...
export default {
    ...
    data: function () {
        return {
            ...
            pageTitle: "Employee Transaction Details",
    ...
    methods: {
        loadEntityData(entityId) {
            ...
            getEmployeeTransactionById(entityId).then(response => {
                this.loadedEntity = JSON.parse(JSON.stringify(response));
                if(this.loadedEntity != null) {
                    this.entityTitle = this.loadedEntity.employee.employeeFullName + ' ( ' + this.loadedEntity.employeeTransactionType.employeeTransactionType + ' )';
                    this.entityDetailsTable.items = [
                        { infoLabel: "Transaction Date", infoValue: formatDate(this.loadedEntity.transactionDate) },
                        { infoLabel: "Employee Name", infoValue: this.loadedEntity.employee.employeeFullName },
                        { infoLabel: "Transaction Type", infoValue: this.loadedEntity.employeeTransactionType.employeeTransactionType },
