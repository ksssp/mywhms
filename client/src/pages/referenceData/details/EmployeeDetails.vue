...
export default {
    ...
    data: function () {
        return {
            ...
            loadedEntity: null,
            loadedEntityId: null,
            entityDoesNotExistMessage: "The employee you are looking for does not exist.",
            editMode: false,
            submitMode: "update",
            entityDetailsTable: {
                fields: [
                ...
                items: [       // update entity specific json data in an array format - Employee
                    { infoLabel: "Employee Code", infoValue: "" },
                    { infoLabel: "Employee Full Name", infoValue: "" },
                    { infoLabel: "Designation", infoValue: "" },
                    { infoLabel: "Monthly Salary", infoValue: "" }
                ]
            }
        };
    },
    ...
    methods: {
        loadEntityData(entityId) {
            ...
            getEmployeeById(entityId).then(response => {
                this.loadedEntity = JSON.parse(JSON.stringify(response));
                if(this.loadedEntity != null) {
                    this.entityTitle = this.loadedEntity.employeeFullName;
                    this.entityDetailsTable.items[0].infoValue = this.loadedEntity.employeeCode;
                    this.entityDetailsTable.items[1].infoValue = this.loadedEntity.employeeFullName;
                    this.entityDetailsTable.items[2].infoValue = this.loadedEntity.designation;
                    this.entityDetailsTable.items[3].infoValue = this.loadedEntity.monthlySalary;
                }
            });
        },
        editLoadedEntity() {
        ...
        deleteLoadedEntity(){
            this.$bvModal.msgBoxConfirm('Are you sure you want to mark the ' + this.entityName + ' as inactive?', {
                ...
                if(value==true) {
                    // delete entity data at the backend: entity - Employee
                    deleteEmployee(this.loadedEntityId).then(response => {
