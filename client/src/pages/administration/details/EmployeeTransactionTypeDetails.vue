                ...
                <div class="card-body" v-else>
                    ...
                    <b-tabs class="tickets-tab-switch">
                        <b-tab title="Overview" active>
                            ...
                            <div class="mx-4">
                                <EmployeeTransactionTypeForm v-if="editMode" :form-data="loadedEntity" :submit-mode="submitMode" 
                                    @saved="saved" @cancelForm="editCancelled"></EmployeeTransactionTypeForm>
...
import { getEmployeeTransactionTypeById, updateEmployeeTransactionType } from "@/services/employeeTransactionType.service";
...
export default {
    ...
    data: function () {
        return {
            entityName: "Employee Transaction Type",
            entityNamePlural: "Employee Transaction Types",
            moduleTitle: "Administration",
            pageTitle: "Employee Transaction Type Details",
            entityListingUrl: "/administration/employeeTransactionTypes/",
            entityTitle: "",
            loadedEntity: {},
            loadedEntityId: null,
            loadedEntityIsActive: false,
            entityDoesNotExistMessage: "The Employee Transaction Type you are looking for does not exist.",
            editMode: false,
            submitMode: "update",
            entityDetailsTable: {
                fields: [
                ...
                items: [       // update entity specific json data in an array format - EmployeeTransactionType
                    { infoLabel: "Employee Transaction Type", infoValue: "" },
                    { infoLabel: "Debit Allowed", infoValue: "" },
                    { infoLabel: "Credit Allowed", infoValue: "" },
    ...
    methods: {
        loadEntityData(entityId) {
            // load entity data from backend: entity - EmployeeTransactionType
            getEmployeeTransactionTypeById(entityId).then(response => {
                this.loadedEntity = JSON.parse(JSON.stringify(response));
                if(this.loadedEntity != null) {
                    this.entityTitle = this.loadedEntity.employeeTransactionType;
                    ...
                    this.entityDetailsTable.items = [
                        { infoLabel: "EmployeeTransaction Type", infoValue: this.loadedEntity.employeeTransactionType },
                        { infoLabel: "Debit Allowed", infoValue: this.loadedEntity.debitAllowed ? 'Yes' : 'No' },
                        { infoLabel: "Credit Allowed", infoValue: this.loadedEntity.creditAllowed ? 'Yes' : 'No' },
        ...
        deleteLoadedEntity(){
            this.$bvModal.msgBoxConfirm('Are you sure you want to mark the ' + this.entityName + ' as inactive?', {
                ...
                if(value==true) {
                    // mark entity data at the backend as inactive: entity - EmployeeTransactionType
                    this.loadedEntity.activeUntil = DateTime.now();
                    updateEmployeeTransactionType(this.loadedEntityId, this.loadedEntity).then(response => {
