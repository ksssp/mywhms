                ...
                <div class="card-body" v-else>
                    ...
                    <b-tabs class="tickets-tab-switch">
                        <b-tab title="Overview" active>
                            ...
                            <div class="mx-4">
                                <ReceiptTypeForm v-if="editMode" :form-data="loadedEntity" :submit-mode="submitMode" 
                                    @saved="saved" @cancelForm="editCancelled"></ReceiptTypeForm>
...
import { getReceiptTypeById, updateReceiptType } from "@/services/receiptType.service";
...
export default {
    ...
    data: function () {
        return {
            entityName: "ReceiptType",
            entityNamePlural: "ReceiptTypes",
            moduleTitle: "Administration",
            pageTitle: "ReceiptType Details",
            entityListingUrl: "/administration/receiptTypes/",
            entityTitle: "",
            loadedEntity: {},
            loadedEntityId: null,
            loadedEntityIsActive: false,
            entityDoesNotExistMessage: "The Receipt Type you are looking for does not exist.",
            editMode: false,
            submitMode: "update",
            entityDetailsTable: {
                fields: [
                ...
                items: [       // update entity specific json data in an array format - ReceiptType
                    { infoLabel: "Receipt Type", infoValue: "" },
                    { infoLabel: "Receipt Category", infoValue: "" },
                    { infoLabel: "Receipt Source", infoValue: "" },
    ...
    methods: {
        loadEntityData(entityId) {
            // load entity data from backend: entity - ReceiptType
            getReceiptTypeById(entityId).then(response => {
                this.loadedEntity = JSON.parse(JSON.stringify(response));
                if(this.loadedEntity != null) {
                    this.entityTitle = this.loadedEntity.receiptType;
                    ...
                    this.entityDetailsTable.items = [
                        { infoLabel: "Receipt Type", infoValue: this.loadedEntity.receiptType },
                        { infoLabel: "Receipt Category", infoValue: this.loadedEntity.receiptCategory },
                        { infoLabel: "Receipt Source", infoValue: this.loadedEntity.receiptSource },
        ...
        deleteLoadedEntity(){
            this.$bvModal.msgBoxConfirm('Are you sure you want to mark the ' + this.entityName + ' as inactive?', {
                ...
                if(value==true) {
                    // mark entity data at the backend as inactive: entity - ReceiptType
                    this.loadedEntity.activeUntil = DateTime.now();
                    updateReceiptType(this.loadedEntityId, this.loadedEntity).then(response => {
