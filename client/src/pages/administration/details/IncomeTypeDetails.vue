                ...
                <div class="card-body" v-else>
                    ...
                    <b-tabs class="tickets-tab-switch">
                        <b-tab title="Overview" active>
                            ...
                            <div class="mx-4">
                                <IncomeTypeForm v-if="editMode" :form-data="loadedEntity" :submit-mode="submitMode" 
                                    @saved="saved" @cancelForm="editCancelled"></IncomeTypeForm>
...
import { getIncomeTypeById, updateIncomeType } from "@/services/incomeType.service";
...
export default {
    ...
    data: function () {
        return {
            entityName: "IncomeType",
            entityNamePlural: "IncomeTypes",
            moduleTitle: "Administration",
            pageTitle: "IncomeType Details",
            entityListingUrl: "/administration/incomeTypes/",
            entityTitle: "",
            loadedEntity: {},
            loadedEntityId: null,
            loadedEntityIsActive: false,
            entityDoesNotExistMessage: "The Income Type you are looking for does not exist.",
            editMode: false,
            submitMode: "update",
            entityDetailsTable: {
                fields: [
                ...
                items: [       // update entity specific json data in an array format - IncomeType
                    { infoLabel: "Income Type", infoValue: "" },
                    { infoLabel: "Income Category", infoValue: "" },
                    { infoLabel: "Income Source", infoValue: "" },
    ...
    methods: {
        loadEntityData(entityId) {
            // load entity data from backend: entity - IncomeType
            getIncomeTypeById(entityId).then(response => {
                this.loadedEntity = JSON.parse(JSON.stringify(response));
                if(this.loadedEntity != null) {
                    this.entityTitle = this.loadedEntity.incomeType;
                    ...
                    this.entityDetailsTable.items = [
                        { infoLabel: "Income Type", infoValue: this.loadedEntity.incomeType },
                        { infoLabel: "Income Category", infoValue: this.loadedEntity.incomeCategory },
                        { infoLabel: "Income Source", infoValue: this.loadedEntity.incomeSource },
        ...
        deleteLoadedEntity(){
            this.$bvModal.msgBoxConfirm('Are you sure you want to mark the ' + this.entityName + ' as inactive?', {
                ...
                if(value==true) {
                    // mark entity data at the backend as inactive: entity - IncomeType
                    this.loadedEntity.activeUntil = DateTime.now();
                    updateIncomeType(this.loadedEntityId, this.loadedEntity).then(response => {
